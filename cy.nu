...
export def 'graph-append-related' [
    ...
    $links
    | dfr append --col (append_related from --step ($step))
    | dfr append --col (append_related to --step ($step + 1))
    | dfr sort-by [link_local_index height]
    | dfr into-lazy
...
export def 'graph-to-gephi' [] {
    ...
    let $t1_height_index = $links.height
        | dfr append --col $particles.height # Particles might be created before they appear in the filtered graph
        | dfr unique
        | dfr with-column (
